/*
Question 9
Write a Dart program that removes duplicate items from a list using a Set. Compare the unique
count with the original list length and print a message if duplicates were removed.
 */
void main() {
  List<int> numbers = [1, 2, 3, 4, 1, 2, 3, 3, 3];
  Set numberUniq = numbers.toSet();
  bool isRemoveduplicate = numbers.length > numberUniq.length;
  if (isRemoveduplicate) {
    print('the value duplicates were removed');
  }
}
